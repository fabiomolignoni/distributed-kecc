...
public class ConnectivityVertexInputFormat
        extends TextVertexInputFormat<IntWritable, ConnectivityStateWritable, IntWritable> {
    ...
    public class IntIntNullVertexReader extends TextVertexReaderFromEachLineProcessed<String[]> {
        ...
        protected Iterable<Edge<IntWritable, IntWritable>> getEdges(String[] tokens) throws IOException {
            List<Edge<IntWritable, IntWritable>> edges = Lists.newArrayListWithCapacity(tokens.length - 1);
            for (int n = 1; n < tokens.length; n++) {
                IntWritable to = new IntWritable(Integer.parseInt(tokens[n]));
                edges.add(EdgeFactory.create(to, new IntWritable(0)));
            }
            return edges;
        }
    }
